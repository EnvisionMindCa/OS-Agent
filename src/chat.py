...
class ChatSession:
    ...
    async def _handle_tool_calls(
        ...
        for call in response.message.tool_calls:
            if call.function.name == "add_two_numbers":
                ...
                nxt_content = (
                    json.dumps(
                        [tc.model_dump() for tc in nxt.message.tool_calls]
                    )
                    if nxt.message.tool_calls
                    else nxt.message.content
                )
                Message.create(
                    conversation=conversation,
                    role="assistant",
    ...
    async def chat(self, prompt: str) -> str:
        ...
        content = (
            json.dumps([tc.model_dump() for tc in response.message.tool_calls])
